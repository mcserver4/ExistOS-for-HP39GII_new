...
void bsp_keyboard_init()
{
    ...
    unsigned int tmp_DOUT, tmp_DOE, tmp_D;
...
void bsp_scan_key(volatile char tab[KEY_COLS][KEY_ROWS])
{
    for(int y = 0; y < KEY_ROWS - 1; y++)
    {
        set_row_line(y); 
        for(int x = 0; x < KEY_COLS; x++)
        {
            tab[x][y] = !read_col_line(x);
        } 
    }
    
    tab[0][10] = ((BF_RD(PINCTRL_DIN0, DIN) >> 14) & 1);
}

bool bsp_query_key(int x, int y)
{
    set_row_line(y);
    return !read_col_line(x);
}

bool bsp_is_key_down(Keys_t key) 
{ 
    if(key == KEY_ON)
    {

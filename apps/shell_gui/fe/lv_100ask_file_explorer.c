...
#define FILE_EXPLORER_QUICK_ACCESS_AREA_WIDTH   (30)
...
static void lv_100ask_file_explorer_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
   ...
   lv_obj_set_size(explorer->brower_area, LV_PCT(100), LV_PCT(100));
    ...
    lv_obj_set_style_bg_color(lv_list_add_text(explorer->list_device, "DEVICES"), lv_palette_main(LV_PALETTE_ORANGE), 0);

    btn = lv_list_add_btn(explorer->list_device, NULL, LV_SYMBOL_DRIVE " Flash");
    lv_obj_add_event_cb(btn, quick_access_event_handler, LV_EVENT_CLICKED, obj);

    btn = lv_list_add_btn(explorer->list_device, NULL, LV_SYMBOL_DRIVE " Usb Disk");
    lv_obj_add_event_cb(btn, quick_access_event_handler, LV_EVENT_CLICKED, obj);
    ...
    lv_obj_set_style_bg_color(lv_list_add_text(explorer->list_places, "ACCESS"), lv_palette_main(LV_PALETTE_LIME), 0);
//
    ...
    lv_label_set_text(explorer->path_label, LV_SYMBOL_EYE_OPEN"/");
...
static void init_style(lv_obj_t * obj)
{
    ...
    lv_obj_set_style_text_color(explorer->quick_access_ctrl_btn, lv_color_hex(0xd0d0d0), 0);
...
static void quick_access_event_handler(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_CLICKED) {
        ...
        else if((strcmp(label_text, LV_SYMBOL_DRIVE " Flash") == 0)) {
#if LV_USE_FS_WIN32
            path = NULL;
#else
            char * tmp_str = "A:";
...
static void brower_file_event_handler(lv_event_t * e)
{
    ...
    static uint16_t cur_select_col;
    static uint16_t cur_select_row;
    ...
    {
        lv_table_get_selected_cell(explorer->file_list, &cur_select_row, &cur_select_col);
    }else if(code == LV_EVENT_DRAW_PART_BEGIN)
    {
        lv_obj_draw_part_dsc_t * dsc = lv_event_get_draw_part_dsc(e);
        if(dsc->part == LV_PART_ITEMS) {
            uint32_t row = dsc->id /  lv_table_get_col_cnt(explorer->file_list);
            uint32_t col = dsc->id - row * lv_table_get_col_cnt(explorer->file_list);
            { 
                if(cur_select_row==row)
                {
                    dsc->rect_dsc->bg_color = lv_color_make(22,22,22);
...
static void show_dir(lv_obj_t * obj, char * path)
{
    ...
    while(1) {
        ...
        else if ((is_end_with(fn , ".exp") == true) || (is_end_with(fn , ".EXP") == true)) {
            lv_table_set_cell_value_fmt(explorer->file_list, index, 0, LV_SYMBOL_PLAY "  %s", fn);
            lv_table_set_cell_value(explorer->file_list, index, 1, "3");
        }
    ...
    lv_100ask_file_explorer_set_sort(obj, LV_100ASK_EXPLORER_SORT_NAME);
    // 让table移动到最顶部
